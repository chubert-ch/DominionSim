...
public class DomGui extends JFrame implements ActionListener {
    ...
    private void initializeComponent() {
        ...
        myTopSplit = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, true, new JScrollPane(getControlPanel()),
                myVPLineChart.getChartPanel());
        myBottomSplit = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, true, myBarChart.getChartPanel(),
                myMoneyLineChart.getChartPanel());
    ...
    private JButton getEditCreateButton(JButton theBotSelector) {
        ...
        if (theBTN == null) {
            ...
            theBTN.setToolTipText(
                    "Edit or create a strategy from an existing strategy, or if nothing selected, from scratch");
    ...
    public void actionPerformed( ActionEvent aE ) {
        ...
        if (aE.getActionCommand().equals("CopyPaste")) {
            ...
            if (theStrategy == null) {
                DomPlayer theNewPlayer = myEngine
                        .loadUserBotsFromXML(new InputSource(new StringReader(getClipboard())));
                if (theNewPlayer != null)
                    refreshBotSelectors(theNewPlayer);
            } else {
                setClipboard(theStrategy.getXML());
                JOptionPane.showMessageDialog(this,
                        "XML-code has been loaded in the clipboard (to share on the internet, via mail...)", "",
    ...
    public String getClipboard() {
        ...
        try {
            if (t != null && t.isDataFlavorSupported(DataFlavor.stringFlavor)) {
                ...
                JOptionPane.showMessageDialog(this, "Your clipboard does not contain a valid XML!", "",
    ...
    public ArrayList<DomPlayer> initPlayers() {
        ...
        for (JButton theSelector : myBotSelectors ) {
            ...
            DomPlayer thePlayer = theSelectedBot
                    .getCopy(theSelectedBot + "(Plr " + (myBotSelectors.indexOf(theSelector) + 1) + ")");
            int j = 0;
