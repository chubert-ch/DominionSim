...
public class DomBuyConditionPanel extends JPanel implements ItemListener {
    ...
    public DomBuyCondition getBuyCondition() {
        ...
        DomBuyCondition theCondition = new DomBuyCondition(theLeftFunction,
                (DomCardName) myLeftCardBox.getSelectedItem(), (DomCardType) myLeftTypeBox.getSelectedItem(),
                theLeftFunction == DomBotFunction.constant ? myLeftFunctionBox.getSelectedItem().toString() : "0",
                (DomBotComparator) myComparatorBox.getSelectedItem(), theRightFunction,
                (DomCardName) myRightCardBox.getSelectedItem(), (DomCardType) myRightTypeBox.getSelectedItem(),
                theRightFunction == DomBotFunction.constant ? myRightFunctionBox.getSelectedItem().toString() : "0",
                (DomBotOperator) myExtraOperator.getSelectedItem(), myExtraValue.getSelectedItem().toString());
        return theCondition;
    }
}